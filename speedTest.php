...
function randomString($length) {
	$characters = 'QAZWSXEDCRFVTGBYHNUJMIKOLP!@~#$%^&*()_+-=|1234567890`;:abcdefghijklmnopqrstuvwxyz';
...
// Might not work if Fastcgi is installed; disable gzip so that the traffic is exact instead of being compressed
disable_gzip();
...
if ((isset($_SESSION['clientKey']))&&(isset($_GET["pg"]))){
	if (($_SESSION['clientKey']!=0)&&($_GET["pg"]=="result")){
		if (isset($_SESSION['timer'])){
			$text="";
			$_SESSION['clientKey']=0;
			$n=1;
			$tmp="id".$n;//echo $_POST[$tmp]."@@";die();
...
echo $loading;
$randNum1=rand(0,1024*1024*6);
$randNum2=rand(1024*1024*8,1024*1024*14-1);
flush();

// after the test run, scale different size for different user connection speed
...
if (($pSpeed<200)||($_SESSION['mobile']==1)) 
	$n=1;
elseif ($pSpeed<500) 
	$n=1;
elseif ($pSpeed<1000) 
...
$text='';
for ($i=1;$i<=$n;$i++){  // prepare string to be sent
...
$_SESSION['downloadSpeed']=round($size / $deltat, 1); // compute download speed
$time = explode(" ",microtime());
